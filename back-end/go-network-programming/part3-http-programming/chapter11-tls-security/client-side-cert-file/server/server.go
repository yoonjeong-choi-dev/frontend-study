// Copy from tls-server/server.go
package main
...
func NewTLSEchoServer(ctx context.Context, address string,
	maxIdle time.Duration, tlsConfig *tls.Config) *EchoServer {
	return &EchoServer{
		ctx:       ctx,
...
func (s *EchoServer) Ready() {
	if s.ready != nil {
		<-s.ready
	}
}

func (s *EchoServer) ListenAndServeTLS(certFilename, keyFilename string) error {
	if s.addr == "" {
...
func (s *EchoServer) ServeTLS(l net.Listener, certFilename, keyFilename string) error {
	if s.tlsConfig == nil {
