...
var (
	Namespace = flag.String("namespace", "yjService", "metrics namespace")
	Subsystem = flag.String("subsystem", "YJMetricServer", "metrics subsystem")

	// Requests metric interface
	// -> prometheus adapter
	// -> options from prometheus client package
	...
	RequestDurationSummary metrics.Histogram = prometheus.NewSummaryFrom(
		prom.SummaryOpts{
			Namespace: *Namespace,
			Subsystem: *Subsystem,
			Name:      "request_duration_histogram_summary_seconds",
			Help:      "Total duration of all requests with summary",
		},
		[]string{},
	)
)
